...
int main()
{
    ...
    int check = 0;
    int size_array = 10;
    int fail_size_array_1 = MAX_QUANTITY_MESSAGES + 100;
    int fail_size_array_2 = -11;
    ...
    printf("Нажми Enter если запустил alarm manager %d\n", size_array);
    getchar();

    plan(NO_PLAN);

        printf("\n\nТесты с правильными аргументами...\n\n");

        cmp_ok(alarm__create_connection(&test_connection, PUBLISHER, 0), "==", OK, SUCCESS);

        for (int i = 0; i < size_array; i++)
        {
            cmp_ok(alarm__send_message(test_connection, test_message), "==", MESSAGE_SIZE, SUCCESS);
        }

        cmp_ok(alarm__recv_quantity(test_connection), "==", size_array, SUCCESS);
        cmp_ok(alarm__recv_all_message(test_connection, test_message_array, size_array), "==", size_array, SUCCESS);
        cmp_ok(alarm__recv_by_filter(test_connection, test_filter, test_message_array, size_array), "==", size_array, SUCCESS);
        cmp_ok(alarm__destroy_connection(&test_connection), "==", 0, SUCCESS);
        ...
        cmp_ok(alarm__destroy_connection(fail_connection), "==", ER_INVALID_ARG, FAIL_CONNECTION);
